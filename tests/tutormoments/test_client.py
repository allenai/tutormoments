...
def test_run_batch_openai_submits_the_reasoning_and_json_knobs(monkeypatch):
    ...
    a dropped reasoning_effort would run the round on the API default while its
    records named an effort it never used. json_mode has to reach the body too:
    the batch callers parse the response as a JSON object."""
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    submitted = []
    with patch("openai.OpenAI") as MockOpenAI, patch("tutormoments.client.time.sleep"):
        client_obj = MagicMock()
