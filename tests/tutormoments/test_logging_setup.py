"""Tests for tutormoments.logging_setup (console + optional file logging)."""
...
def _restore_logging_state(monkeypatch):
    ...
    pkg_levels = {
        ...
        for name in ("tutormoments", "tutormoments_build")
    }
    yield
...
def test_console_only_by_default():
    ...
    assert logging.getLogger("tutormoments").getEffectiveLevel() == logging.INFO
    assert logging.getLogger("tutormoments_build").getEffectiveLevel() == logging.INFO
...
def test_file_and_console_both_receive(tmp_path, capsys):
    ...
    logging.getLogger("tutormoments.cli").info("hello from the run")
...
def test_file_appends_across_setups(tmp_path):
    ...
    setup_logging(log_file=str(log_file))
    logging.getLogger("tutormoments.cli").info("first run")
    setup_logging(log_file=str(log_file))
    logging.getLogger("tutormoments.cli").info("second run")
    for handler in _our_handlers():
        handler.flush()
...
def test_repeated_setup_does_not_stack_handlers(tmp_path, capsys):
    ...
    logging.getLogger("tutormoments.cli").info("only once")
    for handler in _our_handlers():
        handler.flush()
...
def test_env_var_defaults(tmp_path, monkeypatch):
    ...
    assert logging.getLogger("tutormoments").getEffectiveLevel() == logging.DEBUG
...
def test_explicit_args_win_over_env(tmp_path, monkeypatch):
    ...
    assert logging.getLogger("tutormoments").getEffectiveLevel() == logging.WARNING
...
def test_third_party_info_suppressed():
    ...
    assert logging.getLogger("tutormoments.client").isEnabledFor(logging.INFO)
...
def test_per_run_log_file_captures_and_detaches(tmp_path):
    ...
    with per_run_log_file(str(log_file)):
        logging.getLogger("tutormoments.cli").info("inside the run")
    logging.getLogger("tutormoments.cli").info("after the run")
...
def test_per_run_log_file_isolates_threads(tmp_path):
    ...
    setup_logging()
    log = logging.getLogger("tutormoments.cli")
...
def test_per_run_header_written_to_file_only(tmp_path, capsys):
    ...
    with per_run_log_file(str(log_file), header="Command: tutormoments-build x"):
        pass

    assert "Command: tutormoments-build x" in log_file.read_text(encoding="utf-8")
    assert "Command: tutormoments-build x" not in capsys.readouterr().err
...
def test_per_run_log_works_without_setup_logging(tmp_path):
    ...
    pkg_logger = logging.getLogger("tutormoments")
    pkg_logger.setLevel(logging.NOTSET)
    log_file = tmp_path / "run.log"

    with per_run_log_file(str(log_file)):
        logging.getLogger("tutormoments.cli").info("captured without setup")
...
def test_log_context_prefixes_console_and_file(tmp_path, capsys):
    ...
    with per_run_log_file(str(log_file)):
        with log_context("gpt-5-4/plain"):
            logging.getLogger("tutormoments.cli").info("inside cell")
        logging.getLogger("tutormoments.cli").info("outside cell")

    content = log_file.read_text(encoding="utf-8")
    assert "[gpt-5-4/plain] tutormoments.cli: inside cell" in content
    assert "[gpt-5-4/plain] tutormoments.cli: outside cell" not in content
...
def test_cli_run_parser_accepts_log_flags():
    from tutormoments.cli import _build_parser
